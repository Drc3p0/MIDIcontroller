...
const int buttonPin1 = 2; int MIDInote1 = 65; int buttonState1 = 0; int lastbuttonState1 = 0;
const int buttonPin2 = 3; int MIDInote2 = 67; int buttonState2 = 0; int lastbuttonState2 = 0;
const int buttonPin3 = 4; int MIDInote3 = 69; int buttonState3 = 0; int lastbuttonState3 = 0;
const int buttonPin4 = 5; int MIDInote4 = 71; int buttonState4 = 0; int lastbuttonState4 = 0;
const int buttonPin5 = 6; int MIDInote5 = 73; int buttonState5 = 0; int lastbuttonState5 = 0;
const int buttonPin6 = 7; int MIDInote6 = 75; int buttonState6 = 0; int lastbuttonState6 = 0;
...
MIDIpot myPot1(potPin1, 1); 
MIDIpot myPot2(potPin2, 2); 
MIDIpot myPot3(potPin3, 4); 
MIDIpot myPot4(potPin4, 5); 
MIDIpot myPot5(potPin5, 7); 
MIDIpot myPot6(potPin6, 10); 
...
/////////////trigger MIDI from buttonState1
  ...
  if (buttonState1 != lastbuttonState1) {
    ...
    if (buttonState1 == LOW) {
      ...
      // went from off to on:
      MIDI.sendNoteOn(MIDInote1, velocity, MIDIchannel);
      ...
      // went from on to off:
      MIDI.sendNoteOff(MIDInote1, velocity, MIDIchannel);
...
////////////////////trigger MIDI from buttonState2
  ...
  if (buttonState2 != lastbuttonState2) {
    ...
    if (buttonState2 == LOW) {
      // if the current state is HIGH then the button
      // went from off to on:
      MIDI.sendNoteOn(MIDInote2, velocity, MIDIchannel);
      usbMIDI.sendNoteOn(MIDInote2, velocity, MIDIchannel);
      Serial.println("ON");
    }  else {
      // if the current state is LOW then the button
      // went from on to off:
      MIDI.sendNoteOff(MIDInote2, velocity, MIDIchannel);
...
///////////////////////trigger MIDI from buttonState3
  ...
  if (buttonState3 != lastbuttonState3) {
    ...
    if (buttonState3 == LOW) {
      // if the current state is HIGH then the button
      // went from off to on:
      MIDI.sendNoteOn(MIDInote3, velocity, MIDIchannel);
      usbMIDI.sendNoteOn(MIDInote3, velocity, MIDIchannel);
      Serial.println("ON");
    }  else {
      // if the current state is LOW then the button
      // went from on to off:
      MIDI.sendNoteOff(MIDInote3, velocity, MIDIchannel);
...
//////////////////////trigger MIDI from buttonState4
  ...
  if (buttonState4 != lastbuttonState4) {
    ...
    if (buttonState4 == LOW) {
      // if the current state is HIGH then the button
      // went from off to on:
      MIDI.sendNoteOn(MIDInote4, velocity, MIDIchannel);
      usbMIDI.sendNoteOn(MIDInote4, velocity, MIDIchannel);
      Serial.println("ON");
    }  else {
      // if the current state is LOW then the button
      // went from on to off:
      MIDI.sendNoteOff(MIDInote4, velocity, MIDIchannel);
...
////////////////////////trigger MIDI from buttonState5
  ...
  if (buttonState5 != lastbuttonState5) {
    ...
    if (buttonState5 == LOW) {
      // if the current state is HIGH then the button
      // went from off to on:
      MIDI.sendNoteOn(MIDInote5, velocity, MIDIchannel);
      usbMIDI.sendNoteOn(MIDInote5, velocity, MIDIchannel);
      Serial.println("ON");
    }  else {
      // if the current state is LOW then the button
      // went from on to off:
      MIDI.sendNoteOff(MIDInote5, velocity, MIDIchannel);
...
//////////////////////trigger MIDI from buttonState6
  ...
  if (buttonState6 != lastbuttonState6) {
    ...
    if (buttonState6 == LOW) {
      // if the current state is HIGH then the button
      // went from off to on:
      MIDI.sendNoteOn(MIDInote6, velocity, MIDIchannel);
      usbMIDI.sendNoteOn(MIDInote6, velocity, MIDIchannel);
      Serial.println("ON");
    }  else {
      // if the current state is LOW then the button
      // went from on to off:
      MIDI.sendNoteOff(MIDInote6, velocity, MIDIchannel);
